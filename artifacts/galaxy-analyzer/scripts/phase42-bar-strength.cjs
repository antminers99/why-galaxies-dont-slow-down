...
for (const gal of p11.galaxies) {
  ...
  const SBeff = sp.SBeff || 21.0;

  const isBarred = (T >= 1 && T <= 4) ? 1 : 0;

  let barStrengthT = 0;
  if (T >= 1 && T <= 2) barStrengthT = 3;
  else if (T >= 3 && T <= 4) barStrengthT = 2;
  else if (T >= 5 && T <= 6) barStrengthT = 1;
  else barStrengthT = 0;
  ...
  let peakIdx = 0;
  for (let i = 1; i < gobsArr.length; i++) {
    if (gobsArr[i] > gobsArr[peakIdx]) peakIdx = i;
  }
  const innerRise = sorted.length > 1 ? (sorted[peakIdx].log_g_obs - sorted[0].log_g_obs) / Math.max(0.1, sorted[peakIdx].r - sorted[0].r) : 0;

  let innerBump = 0;
  const innerR = Rdisk * 0.5;
  const veryInner = sorted.filter(p => p.r <= innerR);
  if (veryInner.length >= 2) {
    const gobs_inner = veryInner.map(p => Math.pow(10, p.log_g_obs));
    const gbar_inner = veryInner.map(p => Math.pow(10, p.log_g_bar));
    const ratios = gobs_inner.map((g, i) => g / gbar_inner[i]);
    const maxRatio = Math.max(...ratios);
    const minRatio = Math.min(...ratios);
    innerBump = Math.log10(maxRatio / Math.max(0.01, minRatio));
  }

  const SBcontrast = SBdisk - SBeff;

  const logL = Math.log10(L * 1e9);
  const MstarProxy = L * 1e9 * 0.5;
  const logMstar = Math.log10(MstarProxy);
  const barFraction_mass = MstarProxy > 0 ? Math.min(1, Math.pow(10, logSigma0) * Math.PI * (Rdisk * 0.3 * 1e3)**2 / MstarProxy) : 0;

  galaxyData.push({
    name: gal.name, pts, fit,
    logA0: fit.logA0, a0: fit.a0,
    logMHI, logVflat, rcWiggliness, envCode, logSigma0,
    T, isBarred, barStrengthT,
    innerRise, innerBump,
    SBcontrast, SBdisk, SBeff,
    logL, logMstar,
  });
}
...
log("  CONTEXT:");
log("  SPARC provides Hubble type T. Bars are common in T=1-6 (SB/SAB).");
log("  No direct bar strength measurement. Proxies:");
log("");
log("  1) isBarred: binary (T=1-4 → barred)");
log("  2) barStrengthT: ordinal 0-3 from T type");
log("  3) innerRise: log(gobs) gradient in inner RC (bar signature)");
log("  4) innerBump: log ratio of max/min gobs/gbar within 0.5*Rdisk");
log("  5) SBcontrast: SBdisk - SBeff (central concentration proxy)");
log("  6) T (Hubble type): continuous morphological type");
log("");

const candidates = [
  { name: 'isBarred', arr: galaxyData.map(g => g.isBarred), circ: 'ZERO', note: 'morphological binary' },
  { name: 'barStr_T', arr: galaxyData.map(g => g.barStrengthT), circ: 'ZERO', note: 'ordinal from T type' },
  { name: 'innerRise', arr: galaxyData.map(g => g.innerRise), circ: 'MOD', note: 'inner RC gradient' },
  { name: 'innerBump', arr: galaxyData.map(g => g.innerBump), circ: 'MOD', note: 'inner gobs/gbar variation' },
  { name: 'SBcontrast', arr: galaxyData.map(g => g.SBcontrast), circ: 'ZERO', note: 'disk vs eff SB diff' },
  { name: 'T_type', arr: galaxyData.map(g => g.T), circ: 'ZERO', note: 'Hubble type (external)' },
];
...
log("  MORPHOLOGY DISTRIBUTION:");
const tCounts = {};
for (const g of galaxyData) { tCounts[g.T] = (tCounts[g.T] || 0) + 1; }
log("  T types: " + Object.entries(tCounts).sort((a,b)=>+a[0]-+b[0]).map(([t,n])=>"T="+t+":"+n).join(", "));
const nBarred = galaxyData.filter(g => g.isBarred).length;
log("  Barred (T=1-4): " + nBarred + "/" + N + " (" + (nBarred/N*100).toFixed(0) + "%)");
log("");
...
if (promising2.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO bar strength proxy passes fast screen.");
...
log("    bar strength:     " + verdict);
log("    Next: spiral/warp amplitude");
log("");
...
fs.writeFileSync(path.join(__dirname, '../public/phase42-bar-strength.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase42-bar-strength.json");
