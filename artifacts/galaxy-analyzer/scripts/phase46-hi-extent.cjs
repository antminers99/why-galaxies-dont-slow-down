...
for (const gal of p11.galaxies) {
  ...
  const logRHI = Math.log10(RHI > 0.1 ? RHI : 0.1);

  const RHI_Rdisk = RHI / Rdisk;
  const logRHI_Rdisk = Math.log10(RHI_Rdisk > 0.1 ? RHI_Rdisk : 0.1);

  const rMaxRdisk = rMax / Rdisk;
  const logRmaxRdisk = Math.log10(rMaxRdisk > 0.1 ? rMaxRdisk : 0.1);

  const SigmaHI = MHI_Msun / (Math.PI * (RHI * 1e3) ** 2);
  const logSigmaHI = Math.log10(SigmaHI > 0.01 ? SigmaHI : 0.01);

  const MHI_LB = sp.MHI / L;
  const logMHI_LB = Math.log10(MHI_LB > 0.001 ? MHI_LB : 0.001);

  const rcCoverage = rMax / RHI;
  const logRCcoverage = Math.log10(rcCoverage > 0.01 ? rcCoverage : 0.01);

  const logRmax = Math.log10(rMax > 0.1 ? rMax : 0.1);
  ...
  galaxyData.push({
    ...
    logRHI, logRHI_Rdisk, logRmaxRdisk, logSigmaHI,
    logMHI_LB, logRCcoverage, logRmax, outerFrac,
    Rdisk, RHI, rMax,
  });
}
...
log("");
log("=".repeat(80));
log("  PHASE 46: HI Extent — Last Variable in Gas/Kinematics Door");
...
log("  CONTEXT:");
log("  HI extent determines how far gas-dominated RC extends.");
log("  Larger HI disks sample the RAR at lower g_bar,");
log("  where MOND departures are greatest.");
log("  Question: does HI extent independently affect fitted a0?");
log("");

log("  PROXIES:");
log("  1) logRHI: absolute HI radius");
log("  2) RHI/Rdisk: HI extent relative to stellar disk");
log("  3) Rmax/Rdisk: RC extent relative to stellar disk");
log("  4) Sigma_HI: HI surface density");
log("  5) MHI/LB: gas richness ratio");
log("  6) Rmax/RHI: RC coverage fraction of HI disk");
log("  7) logRmax: absolute RC extent");
log("  8) outerFrac: fraction of RC points in outer half");
log("");

const candidates = [
  { name: 'logRHI', arr: galaxyData.map(g => g.logRHI), circ: 'LOW', note: 'absolute HI radius' },
  { name: 'RHI/Rd', arr: galaxyData.map(g => g.logRHI_Rdisk), circ: 'ZERO', note: 'HI extent / stellar disk' },
  { name: 'Rmax/Rd', arr: galaxyData.map(g => g.logRmaxRdisk), circ: 'LOW', note: 'RC extent / stellar disk' },
  { name: 'Sig_HI', arr: galaxyData.map(g => g.logSigmaHI), circ: 'ZERO', note: 'HI surface density' },
  { name: 'MHI/LB', arr: galaxyData.map(g => g.logMHI_LB), circ: 'ZERO', note: 'gas richness' },
  { name: 'Rmax/RHI', arr: galaxyData.map(g => g.logRCcoverage), circ: 'LOW', note: 'RC coverage of HI' },
  { name: 'logRmax', arr: galaxyData.map(g => g.logRmax), circ: 'LOW', note: 'absolute RC extent' },
  { name: 'outerFrc', arr: galaxyData.map(g => g.outerFrac), circ: 'LOW', note: 'fraction outer RC pts' },
];
...
if (promising2.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO HI extent proxy passes fast screen.");
...
log("=".repeat(80));
log("  GAS/KINEMATICS DOOR — FINAL SCORECARD");
log("=".repeat(80));
log("");
log("  sigma_gas:        FAIL (= 1/Vflat, r=-1.00 collinearity)");
log("  non-circular:     wigRatio CONFIRMED (t=2.1, perm p=0.036*)");
log("  bar strength:     PARTIAL (innerRise marginal)");
log("  spiral/warp:      outerOsc PARTIAL (strong suppressor, perm p=0.006***)");
log("  beam smearing:    nPts borderline (fast OK, deep weak)");
log("  gas disk thick:   FAIL (h_gas=1/Sigma0, genuine proxies fail)");
log("  HI extent:        " + (promising2.length > 0 ? "see deep tests above" : "FAIL (no proxy passes)"));
log("");
log("  CONFIRMED variables from this door: wigRatio (only)");
log("  PARTIAL: outerOsc (suppressor), innerRise (bar)");
log("  BORDERLINE: nPts");
log("");
log("  ══════════════════════════════════════════════════════════");
log("  GAS/KINEMATICS DOOR: CLOSED");
log("  Only wigRatio independently confirmed from this domain.");
log("  ══════════════════════════════════════════════════════════");
log("");
...
fs.writeFileSync(path.join(__dirname, '../public/phase46-hi-extent.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase46-hi-extent.json");
