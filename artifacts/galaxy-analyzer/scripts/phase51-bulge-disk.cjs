...
for (const gal of p11.galaxies) {
  ...
  const SBeff = sp.SBeff || SBdisk;
  const inc = sp.inc || 60;

  const isBulgy = T <= 2 ? 1 : 0;

  const concentration = SBdisk - SBeff;

  const innerGbar = innerPts.length > 0
    ? innerPts.map(p => Math.pow(10, p.log_g_bar)).reduce((s,v)=>s+v,0)/innerPts.length : 1e-12;
  const outerGbar = outerPts.length > 0
    ? outerPts.map(p => Math.pow(10, p.log_g_bar)).reduce((s,v)=>s+v,0)/outerPts.length : 1e-12;
  const gbarConcentration = Math.log10(innerGbar / outerGbar);

  const innerFracLight = (() => {
    const Rd_kpc = Rdisk;
    const r1 = rMax * 0.25;
    return 1 - Math.exp(-r1 / Rd_kpc) * (1 + r1 / Rd_kpc);
  })();
  ...
  const centralExcess = (() => {
    if (sorted.length < 5) return 0;
    const n10 = Math.max(2, Math.floor(sorted.length * 0.1));
    const central = sorted.slice(0, n10);
    const centralBoost = central.map(p => p.log_g_obs - p.log_g_bar);
    return centralBoost.reduce((s,v)=>s+v,0) / centralBoost.length;
  })();

  const logMstar = Math.log10(L * 1e9 * 0.5);
  const logMbaryon = Math.log10(Mbar_sun);
  const gasFrac = sp.MHI * 1.33e9 / Mbar_sun;

  const stellarDensity = (L * 1e9 * 0.5) / (Math.PI * (Rdisk * 1e3) ** 2);
  const logStellarDensity = Math.log10(stellarDensity > 0.01 ? stellarDensity : 0.01);
  ...
  galaxyData.push({
    ...
    logMeanRun,
    T, isBulgy, concentration, SBdisk,
    gbarConcentration, innerFracLight, rcRise, centralExcess,
    logMstar, logStellarDensity, gasFrac,
  });
}
...
log("");
log("=".repeat(80));
log("  PHASE 51: Internal Structure — Bulge-to-Disk Ratio");
log("  FAST PROTOCOL");
...
log("  PROXIES:");
log("  1) T-type: Hubble type (lower = earlier/bulgier)");
log("  2) isBulgy: binary T<=2 flag");
log("  3) concentr: SBdisk-SBeff (light concentration)");
log("  4) gbarConc: inner/outer g_bar ratio");
log("  5) innFrLt: inner 25% light fraction");
log("  6) rcRise: inner RC rise (mid - early g_obs)");
log("  7) centExc: central boost excess (gobs-gbar at center)");
log("  8) logMstar: stellar mass");
log("  9) logStelD: stellar surface density");
log("  10) gasFrac: gas fraction");
log("");

const candidates = [
  { name: 'T-type', arr: galaxyData.map(g => g.T), circ: 'ZERO', note: 'Hubble type (external)' },
  { name: 'isBulgy', arr: galaxyData.map(g => g.isBulgy), circ: 'ZERO', note: 'T<=2 binary (external)' },
  { name: 'concentr', arr: galaxyData.map(g => g.concentration), circ: 'ZERO', note: 'SBdisk-SBeff (external)' },
  { name: 'gbarConc', arr: galaxyData.map(g => g.gbarConcentration), circ: 'LOW', note: 'inner/outer g_bar ratio' },
  { name: 'innFrLt', arr: galaxyData.map(g => g.innerFracLight), circ: 'ZERO', note: 'inner light fraction' },
  { name: 'rcRise', arr: galaxyData.map(g => g.rcRise), circ: 'LOW', note: 'inner RC rise' },
  { name: 'centExc', arr: galaxyData.map(g => g.centralExcess), circ: 'MED', note: 'central boost excess' },
  { name: 'logMstar', arr: galaxyData.map(g => g.logMstar), circ: 'ZERO', note: 'stellar mass (external)' },
  { name: 'logStelD', arr: galaxyData.map(g => g.logStellarDensity), circ: 'ZERO', note: 'stellar density (external)' },
  { name: 'gasFrac', arr: galaxyData.map(g => g.gasFrac), circ: 'ZERO', note: 'gas fraction (external)' },
];
...
if (promising2.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO bulge/disk proxy passes fast screen against working BL.");
  log("  ══════════════════════════════════════════════════════════");
} else {
...
fs.writeFileSync(path.join(__dirname, '../public/phase51-bulge-disk.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase51-bulge-disk.json");
