...
for (const gal of p11.galaxies) {
  ...
  const logRdisk = Math.log10(Rdisk);

  const logRlast = Math.log10(rMax);

  const RsNFW = rMax / 5.0;
  const logRsNFW = Math.log10(RsNFW);

  const RsIso = (() => {
    let rTurnover = rMax * 0.3;
    if (innerPts.length >= 3) {
      const sorted = [...innerPts].sort((a, b) => a.r - b.r);
      let maxGobs = -Infinity;
      let rAtMax = sorted[0].r;
      for (const p of sorted) {
        if (p.log_g_obs > maxGobs) { maxGobs = p.log_g_obs; rAtMax = p.r; }
      }
      if (rAtMax > 0.5) rTurnover = rAtMax;
    }
    return rTurnover;
  })();
  const logRsIso = Math.log10(RsIso > 0.1 ? RsIso : 0.1);

  const RsVrise = (() => {
    if (pts.length < 5) return rMax * 0.2;
    const sorted = [...pts].sort((a, b) => a.r - b.r);
    const vArr = sorted.map(p => Math.sqrt(Math.pow(10, p.log_g_obs) * p.r * 3.086e19) / 1e3);
    const vMax = Math.max(...vArr);
    const v90 = vMax * 0.9;
    for (let i = 0; i < sorted.length; i++) {
      if (vArr[i] >= v90) return sorted[i].r;
    }
    return rMax * 0.5;
  })();
  const logRsVrise = Math.log10(RsVrise > 0.1 ? RsVrise : 0.1);
  ...
  galaxyData.push({
    ...
    Rdisk, logRdisk, rMax, logRlast,
    logRsNFW, logRsIso, logRsVrise, RdiskToRlast,
  });
}
...
const rdiskArr = galaxyData.map(g => g.logRdisk);
const rlastArr = galaxyData.map(g => g.logRlast);
const rsNFWArr = galaxyData.map(g => g.logRsNFW);
const rsIsoArr = galaxyData.map(g => g.logRsIso);
const rsVriseArr = galaxyData.map(g => g.logRsVrise);
const rdRlArr = galaxyData.map(g => g.RdiskToRlast);

const BASELINE = [mhiArr, wigArr, envArr];

log("");
log("=".repeat(80));
log("  PHASE 36: R_s (Scale Radius / Characteristic Radius)");
log("  FAST PROTOCOL — Baseline: MHI + wig + env");
log("  Version " + VERSION + ", " + N + " galaxies");
log("=".repeat(80));
log("");

log("  SCALE RADIUS PROXIES:");
log("  - Rdisk: disk scale length from SPARC (EXTERNAL, not from RC)");
log("  - Rlast: last measured radius (geometric, low circularity)");
log("  - Rs_NFW: Rlast/5 (crude NFW scale radius proxy)");
log("  - Rs_iso: radius of peak g_obs in inner RC (turnover radius)");
log("  - Rs_Vrise: radius where V reaches 90% of Vmax");
log("  - Rdisk/Rlast: compactness ratio");
log("");

log("  CIRCULARITY ASSESSMENT:");
log("  - Rdisk: EXTERNAL (photometric) — ZERO circularity ✓");
log("  - Rlast: geometric extent — LOW circularity ✓");
log("  - Rs_iso, Rs_Vrise: from RC shape — MODERATE circularity");
log("  - Rs_NFW: proportional to Rlast — LOW circularity");
log("");

const candidates = [
  { name: 'Rdisk', arr: rdiskArr, feat: 'logRdisk', circ: 'NONE' },
  { name: 'Rlast', arr: rlastArr, feat: 'logRlast', circ: 'LOW' },
  { name: 'Rs_NFW', arr: rsNFWArr, feat: 'logRsNFW', circ: 'LOW' },
  { name: 'Rs_iso', arr: rsIsoArr, feat: 'logRsIso', circ: 'MOD' },
  { name: 'Rs_Vrise', arr: rsVriseArr, feat: 'logRsVrise', circ: 'MOD' },
  { name: 'Rd/Rl', arr: rdRlArr, feat: 'RdRl', circ: 'LOW' },
];
...
for (const c of candidates) {
  ...
  log("  " + c.name.padEnd(12) + " vs MHI:" + rMhi.toFixed(2).padStart(6) + "  wig:" + rWig.toFixed(2).padStart(6) + "  Vf:" + rVf.toFixed(2).padStart(6) + "  circ:" + c.circ);
}
log("");
...
const allFeatures = {
  ...
  'logVflat': vflatArr, 'logRdisk': rdiskArr, 'logRlast': rlastArr,
  'logRsNFW': rsNFWArr, 'logRsIso': rsIsoArr, 'logRsVrise': rsVriseArr,
  'RdRl': rdRlArr,
};
...
log("  BASELINE: MHI+wig+env = " + baselineGC.gc.toFixed(1) + "% gap closure");
log("");
...
for (const cand of candidates) {
  ...
  log("  │  " + cand.name.padEnd(11) + cand.circ.padEnd(5) +
...
if (promising.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO scale radius proxy passes fast screen.");
...
log("=".repeat(80));
log("  PHASE 36 — FAST PROTOCOL SUMMARY");
log("=".repeat(80));
log("");
...
log("  │  FINAL VERDICT: " + verdict.padEnd(12) + " (best: " + best.name + ", circ: " + best.circ + ")" + " ".repeat(12) + "│");
log("  └──────────────────────────────────────────────────────────────────────┘");
log("");
...
log("    R_s:           " + verdict + " (best=" + best.name + ")");
log("    Next: rho_0 (central density)");
log("");
...
fs.writeFileSync(path.join(__dirname, '../public/phase36-scale-radius.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase36-scale-radius.json");
