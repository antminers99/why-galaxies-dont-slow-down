...
for (const gal of p11.galaxies) {
  ...
  const sigma_HI = 10;
  const h_gas_estimate = sigma_HI ** 2 / (Math.PI * 6.674e-11 * Sigma0_bar * 1.989e30 / (3.086e19) ** 2);
  const h_gas_kpc = Math.abs(h_gas_estimate) > 0.001 ? Math.abs(h_gas_estimate) : 0.5;
  const logHgas = Math.log10(h_gas_kpc > 0.01 ? h_gas_kpc : 0.01);

  const hGasToRdisk = h_gas_kpc / Rdisk;
  const logHgasToRd = Math.log10(hGasToRdisk > 0.001 ? hGasToRdisk : 0.001);

  const flaringProxy = RHI / Rdisk;
  const logFlaring = Math.log10(flaringProxy > 0.1 ? flaringProxy : 0.1);
  ...
  const gasToStar = MHI_Msun * 1.33 / (MstarProxy > 0 ? MstarProxy : 1e6);
  const logGasToStar = Math.log10(gasToStar > 0.001 ? gasToStar : 0.001);

  const SBdisk = sp.SBdisk || 21;
  const diskCompactness = -SBdisk;

  const sorted = [...pts].sort((a, b) => a.r - b.r);
  const outerR = sorted.filter(p => p.r > rMid);
  const outerGobs = outerR.map(p => Math.pow(10, p.log_g_obs));
  const outerGbar = outerR.map(p => Math.pow(10, p.log_g_bar));
  const outerRatios = outerGobs.map((g, i) => g / outerGbar[i]);
  const outerMeanRatio = outerRatios.length > 0
    ? outerRatios.reduce((s,v)=>s+v,0) / outerRatios.length : 1;
  const logOuterBoost = Math.log10(outerMeanRatio > 0.1 ? outerMeanRatio : 0.1);
  ...
  galaxyData.push({
    ...
    logHgas, logHgasToRd,
    logFlaring, logSigmaHI, logGasToStar,
    diskCompactness, logOuterBoost,
    Rdisk, RHI, rMax, inc,
  });
}
...
log("");
log("=".repeat(80));
log("  PHASE 45: Gas Disk Thickness / Flaring");
...
log("  CONTEXT:");
log("  Gas disk thickness affects how gas mass maps to surface density,");
log("  which enters g_bar calculation. Thicker gas disks have lower");
log("  effective surface density → different RAR behavior.");
log("  Flaring: gas disks flare outward → outer g_bar overestimated.");
log("");

log("  PROXIES:");
log("  1) h_gas: estimated gas scale height from sigma^2/(pi*G*Sigma)");
log("  2) h_gas/Rdisk: gas thickness relative to stellar disk");
log("  3) RHI/Rdisk: HI extent ratio (flaring proxy)");
log("  4) Sigma_HI: HI surface density (thinner disk → higher Sigma)");
log("  5) gas/star: gas-to-stellar mass ratio");
log("  6) diskCompact: -SBdisk (disk compactness)");
log("  7) outerBoost: mean gobs/gbar ratio in outer half");
log("");

const candidates = [
  { name: 'h_gas', arr: galaxyData.map(g => g.logHgas), circ: 'LOW', note: 'estimated gas scale height' },
  { name: 'h/Rdisk', arr: galaxyData.map(g => g.logHgasToRd), circ: 'LOW', note: 'relative gas thickness' },
  { name: 'RHI/Rd', arr: galaxyData.map(g => g.logFlaring), circ: 'ZERO', note: 'HI flaring proxy (external)' },
  { name: 'Sig_HI', arr: galaxyData.map(g => g.logSigmaHI), circ: 'ZERO', note: 'HI surface density' },
  { name: 'gas/star', arr: galaxyData.map(g => g.logGasToStar), circ: 'ZERO', note: 'gas fraction (external)' },
  { name: 'diskComp', arr: galaxyData.map(g => g.diskCompactness), circ: 'ZERO', note: '-SBdisk' },
  { name: 'outerBst', arr: galaxyData.map(g => g.logOuterBoost), circ: 'HIGH', note: 'outer gobs/gbar (circular!)' },
];
...
if (promising2.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO gas disk thickness proxy passes fast screen.");
...
fs.writeFileSync(path.join(__dirname, '../public/phase45-gas-disk.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase45-gas-disk.json");
