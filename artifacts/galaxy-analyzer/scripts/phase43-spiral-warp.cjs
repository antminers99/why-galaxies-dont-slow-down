...
for (const gal of p11.galaxies) {
  ...
  const T = sp.T !== undefined ? sp.T : 5;

  const sorted = [...pts].sort((a, b) => a.r - b.r);

  const rarResiduals = sorted.map(p => {
    const gb = Math.pow(10, p.log_g_bar);
    const pred = Math.log10(mcgaughRAR(gb, fit.a0));
    return p.log_g_obs - pred;
  });

  const outerR = sorted.filter(p => p.r > rMid);
  const outerResid = outerR.map(p => {
    const gb = Math.pow(10, p.log_g_bar);
    return p.log_g_obs - Math.log10(mcgaughRAR(gb, fit.a0));
  });

  let outerOscillation = 0;
  if (outerResid.length >= 4) {
    let crossings = 0;
    const outerMean = outerResid.reduce((s,v)=>s+v,0) / outerResid.length;
    const centered = outerResid.map(v => v - outerMean);
    for (let i = 1; i < centered.length; i++) {
      if (centered[i] * centered[i-1] < 0) crossings++;
    }
    outerOscillation = crossings / (centered.length - 1);
  }

  let outerSlope = 0;
  if (outerR.length >= 3) {
    const rr = outerR.map(p => p.r);
    const mr = rr.reduce((s,v)=>s+v,0) / rr.length;
    const mres = outerResid.reduce((s,v)=>s+v,0) / outerResid.length;
    let sxy = 0, sxx = 0;
    for (let i = 0; i < rr.length; i++) { sxy += (rr[i]-mr)*(outerResid[i]-mres); sxx += (rr[i]-mr)**2; }
    outerSlope = sxx > 0 ? sxy / sxx : 0;
  }

  const outerAmplitude = outerResid.length >= 2
    ? Math.max(...outerResid) - Math.min(...outerResid) : 0;

  let gbarSlope = 0;
  if (outerR.length >= 3) {
    const outerLogGbar = outerR.map(p => p.log_g_bar);
    const rr = outerR.map(p => p.r);
    const mr = rr.reduce((s,v)=>s+v,0) / rr.length;
    const mgb = outerLogGbar.reduce((s,v)=>s+v,0) / outerLogGbar.length;
    let sxy = 0, sxx = 0;
    for (let i = 0; i < rr.length; i++) { sxy += (rr[i]-mr)*(outerLogGbar[i]-mgb); sxx += (rr[i]-mr)**2; }
    gbarSlope = sxx > 0 ? sxy / sxx : 0;
  }

  const rcExtent = rMax / Rdisk;
  const logRCextent = Math.log10(rcExtent > 0.1 ? rcExtent : 0.1);

  const lastPts = sorted.slice(-Math.max(3, Math.floor(sorted.length * 0.2)));
  const lastResid = lastPts.map(p => {
    const gb = Math.pow(10, p.log_g_bar);
    return p.log_g_obs - Math.log10(mcgaughRAR(gb, fit.a0));
  });
  const lastMeanResid = lastResid.reduce((s,v)=>s+v,0) / lastResid.length;

  galaxyData.push({
    name: gal.name, pts, fit,
    logA0: fit.logA0, a0: fit.a0,
    logMHI, logVflat, rcWiggliness, envCode, logSigma0,
    outerWig, logOuterWig: Math.log10(outerWig > 1e-5 ? outerWig : 1e-5),
    outerOscillation,
    outerSlope,
    outerAmplitude, logOuterAmp: Math.log10(outerAmplitude > 1e-5 ? outerAmplitude : 1e-5),
    gbarSlope,
    logRCextent,
    lastMeanResid,
    T, Rdisk, rMax,
...
log("");
log("=".repeat(80));
log("  PHASE 43: Spiral / Warp Amplitude");
...
log("  CONTEXT:");
log("  No direct spiral arm or warp measurements in SPARC.");
log("  Proxies from outer RC shape (spirals/warps affect outer disk):");
log("");
log("  PROXIES:");
log("  1) outerWig: outer-half RC wiggliness (residual variability)");
log("  2) outerOscillation: fraction of zero-crossings in outer resid");
log("  3) outerSlope: slope of RAR residuals vs radius in outer half");
log("  4) outerAmplitude: max-min of outer RAR residuals (peak-to-peak)");
log("  5) gbarSlope: slope of log(gbar) vs r in outer half (disk falloff)");
log("  6) rcExtent: Rmax/Rdisk (how far RC extends into outer disk)");
log("  7) lastMeanResid: mean RAR residual in last 20% of RC points");
log("");

const candidates = [
  { name: 'outerWig', arr: galaxyData.map(g => g.logOuterWig), circ: 'MOD', note: 'outer RC scatter' },
  { name: 'outerOsc', arr: galaxyData.map(g => g.outerOscillation), circ: 'MOD', note: 'outer resid zero-crossings' },
  { name: 'outerSlope', arr: galaxyData.map(g => g.outerSlope), circ: 'MOD', note: 'RAR resid trend outer' },
  { name: 'outerAmp', arr: galaxyData.map(g => g.logOuterAmp), circ: 'MOD', note: 'peak-to-peak outer resid' },
  { name: 'gbarSlope', arr: galaxyData.map(g => g.gbarSlope), circ: 'LOW', note: 'baryonic falloff rate' },
  { name: 'rcExtent', arr: galaxyData.map(g => g.logRCextent), circ: 'LOW', note: 'Rmax/Rdisk (external)' },
  { name: 'lastResid', arr: galaxyData.map(g => g.lastMeanResid), circ: 'HIGH', note: 'edge RAR residual (circular!)' },
];
...
if (promising2.length === 0) {
  log("  ══════════════════════════════════════════════════════════");
  log("  NO spiral/warp proxy passes fast screen.");
...
log("  VERDICT: " + verdict);
log("");
log("  Gas/Kinematics Door scorecard:");
log("    sigma_gas:        FAIL (= V_flat)");
log("    non-circular:     wigRatio CONFIRMED");
log("    bar strength:     PARTIAL (innerRise)");
log("    spiral/warp:      " + verdict);
log("    Next: beam smearing / gas disk thickness / HI extent");
log("");
...
fs.writeFileSync(path.join(__dirname, '../public/phase43-spiral-warp.json'), JSON.stringify(output, null, 2));
log("\n  Results saved to public/phase43-spiral-warp.json");
