...
(Table~\ref{tab:regime}). A gradual transition occurs above
$\vflat \sim 120~\kms$, reaching full strength above $\sim$180~$\kms$.
In the low-$\vflat$ regime ($N=10$), the signal is completely unstable
($P(\Delta > 0) = 18.6$ per cent).
...
The empirical law can be stated as follows. Per-galaxy $\azero$ is governed
by a hierarchical, regime-dependent baryon--halo coupling law with three
tiers:
