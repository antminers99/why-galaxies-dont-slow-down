...
>
> **ERRATA (T||5 bug):** The M5 numbers in this document were computed with a JavaScript bug (`T || 5`) that treated morphological type T=0 as falsy for NGC4138 and UGC06786. Corrected M5: Ups_perp coefficient = 0.372 (not 0.658), LOO gap% = 46.6% (not 51.0%), R² = 0.588 (not ~0.73), M3 retains 95% of M5 (not 86%). The numbers below are the original uncorrected values, preserved for historical reference. See `variable_definitions.md` for corrected values.
...
**Verdict.** The per-galaxy $a_0$ is **not needed** for predicting rotation curve shape. The direct law — outer slope as a function of structural properties plus inner $V_\mathrm{max}$ — achieves LOO $R^2 = 0.677$ compared to $0.292$ for the $a_0$-based approach. The $a_0$ framework has been superseded by a more fundamental description: the ratio $V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner}$, which directly encodes halo concentration and determines how much the rotation curve will rise beyond the inner region. Phase 97 confirms this law is genuine.

### 12.19 Is the Direct Law Genuine or Geometric? (Phase 97)

The direct law from Phase 96 uses $V_\mathrm{max}^\mathrm{inner}$ (from the inner RC) to predict outer slope (from the outer RC). Since both come from the same rotation curve, this section tests whether the relationship is a genuine physical law or a geometric tautology. Seven tests are applied ($N = 104$).

**Split-half by construction.** $V_\mathrm{max}^\mathrm{inner}$ is computed from the inner half and outer slope from the outer half — there is zero radial overlap. The raw correlation is $r(\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner}), \mathrm{outerSlope}) = 0.846$. This is the split-half test by design.

**Not all inner velocities work equally.** If the relationship were purely geometric (any inner velocity would predict outer slope), then $V(1\,R_d)$, $V(2\,R_d)$, $V_\mathrm{peak}$, and $V_\mathrm{max}^\mathrm{inner}$ would perform identically. They do not: LOO $R^2$ ranges from $0.345$ ($V_\mathrm{peak}$) to $0.686$ ($V_\mathrm{max}^\mathrm{inner}$). The spread between proxies ($\Delta r = 0.090$) confirms physical content beyond geometry.

**Survives all controls.** After controlling structural variables, morphological type, number of RC points, and quality flag: partial $r(\log V_\mathrm{max}^\mathrm{inner}, \mathrm{outerSlope}) = -0.772$. LOO $R^2 = 0.689$ with eight predictors (gap $< 8\%$).

**The ratio is the fundamental variable.** Comparing $\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner})$ alone (LOO $R^2 = 0.691$) versus $\log V_\mathrm{flat} + \log V_\mathrm{max}^\mathrm{inner}$ as separate predictors (LOO $R^2 = 0.688$): the ratio is sufficient. Adding structural variables to the ratio does not improve prediction (LOO $R^2 = 0.692$). A single number — $\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner})$ — explains $69\%$ of outer slope variance with minimal overfitting.

**Behavior below 70 km/s.** The ratio $V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner}$ fails in the low regime ($r = 0.130$) because $V_\mathrm{flat}$ is ill-defined for dwarfs with rising RCs. However, $\log V_\mathrm{max}^\mathrm{inner}$ alone maintains $r = -0.607$ with outer slope, and the structural + innerVmax model achieves LOO $R^2 = 0.386$ ($N = 39$). Across all 143 galaxies: LOO $R^2 = 0.611$.

**Permutation test.** $r(\log \mathrm{ratio}, \mathrm{outerSlope}) = 0.846$, $p_\mathrm{perm} < 0.0005$ (0/2000 permutations matched).

**Verdict.** The direct law passes all seven tests: 4 genuine markers, 0 geometric markers. The relationship between $\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner})$ and outer slope is a **genuine physical law**, not a geometric artifact. It reflects a real inside-to-outside dynamical coupling: how quickly a galaxy builds its rotation velocity in the inner region determines how much the outer curve will continue to rise. This single ratio — encoding halo concentration — supersedes the per-galaxy $a_0$ as the fundamental variable governing rotation curve shape. Phase 98 tests external replication.

### 12.20 External Replication on N=45 Published-Distance Sample (Phase 98)

Phase 97 confirmed the direct law is genuine on the full SPARC sample. Phase 98 tests whether it replicates on the $N = 45$ quality-controlled subset with published distances, using strict train/test splits.

**The signal is real within N45.** The correlation $r(\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner}), \mathrm{outerSlope}) = 0.625$ within the 42 high-$V_\mathrm{flat}$ N45 galaxies, with permutation $p < 0.0001$. The direction, sign, and significance all replicate.

**Cross-sample prediction is asymmetric.** Training on non-N45 ($N = 62$) and testing on N45 ($N = 42$): test $R^2 = 0.230$, $r(\mathrm{pred}, \mathrm{actual}) = 0.625$, RMSE $= 0.096$. Training on N45 and testing on non-N45: test $R^2 = 0.607$, $r = 0.873$, RMSE $= 0.111$. The asymmetry reflects two factors: (1) the N45 subset has smaller variance in outer slope (published-distance galaxies are more homogeneous), making $R^2$ look smaller even with similar RMSE; (2) the non-N45 training set is larger and more diverse, producing a steeper calibration.

**Coefficient instability.** The slope of the $\log(\mathrm{ratio}) \to \mathrm{outerSlope}$ relationship differs by $45.7\%$ between samples: $b = 1.337$ (N45) vs. $b = 2.129$ (non-N45). The intercepts are consistent ($a \approx 0$). This instability suggests the **functional form** is universal (the ratio predicts outer slope) but the **quantitative calibration** depends on sample composition — possibly reflecting distance-dependent biases in $V_\mathrm{flat}$ or resolution effects on inner $V_\mathrm{max}$.

**Within-N45 LOO.** LOO $R^2 = 0.327$ for the ratio model on N45 alone ($N = 42$). For Q=1 N45 galaxies ($N = 29$): $r = 0.555$, LOO $R^2 = 0.175$. The weaker LOO reflects the restricted dynamic range: the N45 sample spans a narrower range of outer slopes than the full SPARC sample.

**Verdict.** The direct law is **partially replicated**: the qualitative relationship (inner velocity ratio predicts outer slope) holds with high significance ($p < 0.0001$) in the N45 subset, but the quantitative calibration differs between samples. The law explains $\sim 39\%$ of outer slope variance in the N45 sample (from $r^2 = 0.625^2$) compared to $\sim 72\%$ in the full sample. This attenuation is consistent with measurement noise in a restricted-range subsample rather than failure of the underlying physical relationship. The inside-to-outside coupling is robust; its precise quantification requires attention to distance uncertainties and resolution effects.

---

### 12.21 The Calibration Law: Distance Modulates the Slope (Phase 99)

Phase 98 revealed a $45.7\%$ slope difference between N45 and non-N45 samples. Phase 99 asks: **why?**

**Sample composition differs systematically.** The N45 (published-distance) galaxies have $2\times$ more data points (median 29.5 vs. 15), $2\times$ greater radial coverage ($10.6 R_d$ vs. $5.5 R_d$), higher surface density ($\log \Sigma_0 = 0.78$ vs. $0.08$), lower gas fraction ($f_\mathrm{gas} = 0.15$ vs. $0.37$), and higher $V_\mathrm{flat}$ (median $\log V = 2.25$ vs. $2.04$). The N45 subset is not a random draw: it preferentially includes well-resolved, massive spirals.

**Variance is restricted.** The standard deviation of $\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner})$ in N45 is $71\%$ that of non-N45 ($0.052$ vs. $0.073$), and outer slope variance is only $62\%$ ($0.111$ vs. $0.178$). This alone biases OLS slopes and $R^2$ downward in the restricted sample — a textbook range-restriction effect.

**Distance is the primary moderator.** We tested nine candidate moderators via interaction terms ($\mathrm{outerSlope} \sim \log\mathrm{ratio} \times \mathrm{moderator}$). Ranked by $\Delta R^2$: $\log D$ ($+0.056$), radial coverage ($+0.028$), $\log$ radial range ($+0.025$), $\log R_\mathrm{disk}$ ($+0.018$). All others $< 0.005$. The interaction coefficient $\beta_\mathrm{inter} = -1.36$ for distance: **at greater distances, the effective slope is shallower**, consistent with angular resolution degrading the measurement of inner $V_\mathrm{max}$ (beam smearing washes out the inner peak).

**Distance-corrected model.** Including the $\log\mathrm{ratio} \times \log D$ interaction: $R^2 = 0.772$, LOO $R^2 = 0.750$ ($N = 104$) — an improvement from LOO $0.691$ (ratio alone). However, cross-sample transfer remains asymmetric: train non-N45 $\to$ test N45 yields $R^2 = 0.241$; reverse $R^2 = 0.682$. The distance correction accounts for part of the calibration difference but does not fully resolve it, suggesting additional systematic effects (data point density, radial coverage) contribute.

**Robust regression.** Theil-Sen (median-based) slopes: $b = 1.40$ (N45), $b = 2.12$ (non-N45), difference $= 40.9\%$ — slightly less than OLS ($45.7\%$) but still substantial. The difference is not driven by outliers.

**Functional form matters.** Using the linear fraction $V_\mathrm{max}^\mathrm{inner}/V_\mathrm{flat}$ instead of $\log$ worsens instability ($63.5\%$ slope difference) and cross-validation (train non-N45 $\to$ test N45: $R^2 = -0.051$). The logarithmic ratio is the correct functional form.

**Verdict.** The slope instability has a physical explanation: distance-dependent angular resolution degrades inner $V_\mathrm{max}$ measurement, flattening the effective relationship for more distant galaxies. The N45 sample's restricted range compounds this through a standard statistical artifact. A distance-corrected calibration improves LOO $R^2$ from $0.691$ to $0.750$, but full cross-sample transfer requires accounting for radial coverage and data density as well. The calibration equation is:

$$\mathrm{outerSlope} = 0.015 + 3.60 \cdot \log\!\left(\frac{V_\mathrm{flat}}{V_\mathrm{max}^\mathrm{inner}}\right) - 0.001 \cdot \log D - 1.36 \cdot \log\!\left(\frac{V_\mathrm{flat}}{V_\mathrm{max}^\mathrm{inner}}\right) \cdot \log D$$

This means: for a galaxy at $D = 10$ Mpc, the effective slope is $3.60 - 1.36 \times 1.0 = 2.24$; at $D = 100$ Mpc it drops to $3.60 - 1.36 \times 2.0 = 0.88$. The inside-to-outside coupling is real, but its observational expression weakens with distance as resolution degrades.

---

### 12.22 Physical Law Confirmed: Survives All Measurement Controls (Phase 100)

Phase 99 showed that distance modulates the calibration. Phase 100 asks the decisive question: does the signal **survive** when all measurement effects are removed, or does it **disappear**?

**Test 1: Within distance bins.** The sample is divided into tertiles by distance. In every bin, the correlation is massive and highly significant:

| Distance bin | $N$ | $r$ | $p$ |
|:---|:---|:---|:---|
| Near ($D < 14$ Mpc) | 34 | 0.934 | $< 0.0001$ |
| Mid ($14 \leq D < 21$ Mpc) | 34 | 0.782 | $< 0.0001$ |
| Far ($D \geq 21$ Mpc) | 36 | 0.832 | $< 0.0001$ |

In narrow bands: 0–10 Mpc ($r = 0.942$), 10–20 Mpc ($r = 0.787$), 20–40 Mpc ($r = 0.871$), 40+ Mpc ($r = 0.808$). The signal is present at **every** distance.

**Partial correlations controlling for measurement properties.** After controlling for $\log D$: partial $r = 0.843$. After controlling for $\log D$, number of data points, and radial coverage: partial $r = 0.806$. The raw correlation ($r = 0.846$) drops by only $4.7\%$ when all observational properties are removed. Measurement effects account for a negligible fraction of the signal.

**Test 2: Distance-matched pairs.** 51 galaxy pairs matched to within $20\%$ in distance. Within each pair, the galaxy with the higher $\log(V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner})$ has the higher outer slope: $r(\Delta \log\mathrm{ratio}, \Delta \mathrm{outerSlope}) = 0.813$, $p < 0.0001$. This eliminates distance as a confound entirely.

**Test 3: Alternative inner proxies.** Five inner velocity measures, from most to least resolution-sensitive, all show significant correlations after controlling for distance, $N_\mathrm{pts}$, and radial coverage:

| Proxy | $r$ (raw) | partial $r$ (all controls) |
|:---|:---|:---|
| InnerVmax (original) | 0.846 | 0.806 |
| Mean inner $V$ | 0.777 | 0.724 |
| Deep inner Vmax (inner 50%) | 0.727 | 0.663 |
| Inner baryon $V_\mathrm{max}$ | 0.594 | 0.529 |
| $V$ at $1 R_d$ | 0.588 | 0.591 |

Every proxy achieves partial $r > 0.5$ after all measurement controls. The inner baryon $V_\mathrm{max}$ (derived purely from mass decomposition, not observed velocities) still correlates at $r = 0.529$ — this proxy is completely independent of beam smearing, confirming the signal has a physical origin in the baryonic mass distribution.

**The inner mass discrepancy** ($\log(V_\mathrm{obs}^\mathrm{inner}/V_\mathrm{bar}^\mathrm{inner})$) correlates with outer slope at $r = 0.364$ ($p < 0.001$), partial $r = 0.299$ after all controls. This is weaker but significant: galaxies with more dark matter dominance in the inner region have steeper outer slopes — a signature of halo concentration.

**Verdict.** Physical score: **8/8**. Measurement score: **0/8**. The inside-to-outside dynamical coupling is a **genuine physical law**, not a measurement artifact. Distance modulates the calibration (Phase 99) but does not create the signal. The law survives within every distance bin ($r > 0.78$), within distance-matched pairs ($r = 0.81$), with alternative inner proxies (partial $r > 0.5$), and after controlling for all observational quality indicators (partial $r = 0.81$). The ratio $V_\mathrm{flat}/V_\mathrm{max}^\mathrm{inner}$ encodes a real physical quantity — halo concentration — that determines rotation curve shape independently of how well the galaxy is observed.
...
| **Direct law is GENUINE, not geometric** | **Phase 97: 4 genuine markers, 0 geometric; proxy spread=0.090; p<0.0005** | **Confirmed** |
| **log(Vflat/InnerVmax) is THE variable** | **Single ratio: LOO R²=0.691; adding structural vars: 0.692 (no improvement)** | **Key finding** |
| **Not all inner velocities work equally** | **LOO R² ranges 0.345 (Vpeak) to 0.686 (InnerVmax); NOT geometric** | **Confirmed** |
| **Law survives all controls** | **Partial r=−0.772 after struct + T + nPts + Q; LOO R²=0.689** | **Confirmed** |
| **Works across ALL 143 galaxies** | **Struct + innerVmax: LOO R²=0.611 (all); 0.386 (dwarfs alone)** | **Key finding** |
| **Direct law partially replicates on N45** | **Phase 98: r=0.625 within N45 (p<0.0001); test R²=0.23 (train non-N45→N45)** | **Partial** |
| **Qualitative relationship is universal** | **Sign, direction, significance replicate; calibration differs 45.7% (restricted range)** | **Confirmed** |
| **Reverse replication strong** | **Train N45→test non-N45: R²=0.607, r=0.873** | **Key finding** |
| **Slope instability explained by distance** | **Phase 99: logD interaction ΔR²=0.056; beam smearing flattens inner Vmax** | **Key finding** |
| **Distance-corrected model improves** | **LOO R²=0.750 (corrected) vs 0.691 (uncorrected); N=104** | **Confirmed** |
| **N45 range restriction is real** | **sd(logRatio) 71% of non-N45; sd(outerSlope) 62% — textbook attenuation** | **Confirmed** |
| **Log form is correct** | **Linear fraction worsens: 63.5% slope diff, cross-val R²=-0.051** | **Confirmed** |
| **LAW IS PHYSICAL, NOT MEASUREMENT** | **Phase 100: 8/8 physical, 0/8 measurement; survives ALL controls** | **CONFIRMED** |
| **Survives within every distance bin** | **Near r=0.934, Mid r=0.782, Far r=0.832; all p<0.0001** | **Key finding** |
| **Survives distance-matched pairs** | **51 pairs: r(Δratio,Δslope)=0.813, p<0.0001** | **Key finding** |
| **Partial r after ALL controls = 0.806** | **Raw 0.846 drops only 4.7% controlling D, nPts, radCov** | **Key finding** |
| **Alternative proxies all survive** | **Mean V: 0.724, baryon Vmax: 0.529 after controls** | **Key finding** |
