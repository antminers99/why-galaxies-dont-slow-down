...
The $\mtwo$ power correlates strongly with bilateral excess (Fig.~\ref{fig:dq_m2}):
\begin{equation}
...
\caption{Bilateral excess ($\DQ$) versus $\log_{10}$ $\mtwo$ azimuthal power for $N = 7$ THINGS galaxies ($r = 0.85$, $p = 0.005$). NGC~2841 (red, high-$H$) and NGC~5055 (dark blue, low-$H$) anchor the extremes despite near-identical baryonic masses. The dashed line is the OLS fit.}
\label{fig:dq_m2}
\end{figure}
...
\caption{Property comparison of the gold pair: NGC~2841 (high bilateral excess) and NGC~5055 (low bilateral excess). Despite closely matched baryonic mass ($\Delta\log M_\mathrm{bar} = 0.07$) and flat velocity ($\Delta V_\mathrm{flat} = 106\kms$), the $\mtwo$ power differs by $10.5\times$, confirming that halo angular structure --- not baryonic properties --- drives the bilateral coupling.}
\label{fig:goldpair}
\end{figure}
...
We subject the carrier claim to 11 independent tests designed to break it (Fig.~\ref{fig:redteam}).
All tests pass.
...
The complete causal chain connecting formation history to the observed bilateral coupling (Fig.~\ref{fig:causal}) is:
\begin{equation*}
...
\caption{Proposed causal chain from formation history to bilateral coupling. Halo triaxiality generates both an $\mtwo$ velocity field mode (angular, 1D-invisible) and an enclosed mass shift (integral, 1D-visible). The mass shift drives small correlated perturbations to $V_\mathrm{flat}$ and $a_0$ with a $4{:}1$ asymmetry. The $\mtwo$ mode provides the diagnostic fingerprint: $r(\DQ, P_2) = 0.85$.}
\label{fig:causal}
\end{figure}
